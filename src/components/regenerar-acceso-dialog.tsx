...
import { regenerarAcceso } from "@/lib/actions";
...
export function RegenerarAccesoDialog({ professionalId, name }: Props) {
  ...
  function handleGenerar() {
    ...
    startTransition(async () => {
      const res = await regenerarAcceso(professionalId);
      if (res.ok) {
        setLink(res.link);
      } else {
        setError(res.error);
  ...
  function handleWhatsApp() {
    if (!link) return;
    const msg = `Hola ${name}, te comparto un nuevo link de acceso a Breteame. Es de un solo uso:\n\n${link}`;
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); else setOpen(true); }}>
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            {link
              ? "Mandá el link por donde quieras. El anterior deja de funcionar."
              : "Generá un nuevo link de acceso. El anterior deja de funcionar."}
          </DialogDescription>
        </DialogHeader>
